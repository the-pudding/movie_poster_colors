...
write.csv(colorsMeta, here::here("analysis", "processed_data", "colorsMeta.csv"), row.names = FALSE)
```
...
Let's try breaking out the data by genre. 

```{r}
genre <- colorsMeta %>% 
  separate_rows(genres, sep = ",") %>% 
  mutate(genres = (tolower(trimws(genres)))) %>% 
  filter(!is.na(genres),
         genres != "\n",
         genres != "undefined") %>% 
  filter(Year >= 1960)
...
write.csv(genre, here::here("src", "assets", "data", "genreColors.csv"), row.names = FALSE)

ggplot(genre, aes(x = Year)) + geom_histogram()
```
...
```{r}
resizeImages <- function(file){
  image <- load.image(here::here("analysis", "high-res", "pudding", file))
  resized <- imresize(image, scale = 0.1)
  
  save.image(resized, file = here::here("analysis", "low-res", file))
}

map(last$fileName, resizeImages)
```
